...
class BookingApiIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired private ExpiredBookingCleanupJob expiredBookingCleanupJob;
    ...
    void scheduledCleanupExpiresOnlyDueInitiatedBookingsAndIsRepeatable() throws Exception {
        ...
        Long expiredId =
                createBooking(
                        expiredContext.customerToken(),
                        expiredContext.show().getId(),
                        expiredSeat.getId());
        ...
        Long validId =
                createBooking(
                        validContext.customerToken(),
                        validContext.show().getId(),
        ...
        assertThat(bookingRepository.findById(expiredId).orElseThrow().getStatus())
                .isEqualTo(BookingStatus.EXPIRED);
        assertThat(bookingRepository.findById(validId).orElseThrow().getStatus())
    ...
    void concurrentCustomersCannotCreateTwoBookingsForTheSameSeat() throws Exception {
        ...
        String body =
                json(Map.of("showId", context.show().getId(), "seatIds", List.of(seat.getId())));
    ...
    void bookingReferenceSupportsOwnedAndManagementLookups() throws Exception {
        ...
        mockMvc.perform(
                        get(
                                        "/api/customer/bookings/reference/{reference}",
                                        booking.getBookingReference())
                                .header("Authorization", bearer(context.customerToken())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.bookingReference").value(booking.getBookingReference()))
        ...
        mockMvc.perform(
                        get(
                                        "/api/staff/bookings/reference/{reference}",
                                        booking.getBookingReference())
                                .header("Authorization", bearer(staffToken)))
                .andExpect(status().isOk());
        mockMvc.perform(
                        get(
                                        "/api/admin/bookings/reference/{reference}",
                                        booking.getBookingReference())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk());
        mockMvc.perform(get("/api/customer/bookings/reference/UNKNOWN"))
                .andExpect(status().isUnauthorized());
        mockMvc.perform(
                        get("/api/admin/bookings/reference/UNKNOWN")
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isNotFound());
    }
    ...
    void bookingPricingIsAnImmutableExactSnapshot() throws Exception {
        ...
        Long bookingId =
                createBooking(
                        context.customerToken(), context.show().getId(), seats.getFirst().getId());
        Booking booking = bookingRepository.findById(bookingId).orElseThrow();
        var bookingSeats = bookingSeatRepository.findByBookingId(bookingId);
        assertThat(booking.getTotalAmount())
                .isEqualByComparingTo(bookingSeats.getFirst().getUnitPrice());
        ...
        mockMvc.perform(
                        get("/api/customer/bookings/{id}", bookingId)
                                .header("Authorization", bearer(context.customerToken())))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.data.totalAmount")
                                .value(booking.getTotalAmount().doubleValue()))
                .andExpect(
                        jsonPath("$.data.selectedSeats[0].price")
                                .value(bookingSeats.getFirst().getUnitPrice().doubleValue()));
    }
    ...
    void staleInitiatedBookingExpiresLazilyAndReleasesReservedSeats() throws Exception {
        ...
        Seat seat = seatsForShow(context.show().getId()).getFirst();
        Long bookingId =
                createBooking(context.customerToken(), context.show().getId(), seat.getId());
        Booking booking = bookingRepository.findById(bookingId).orElseThrow();
        ...
        assertThat(seatRepository.findById(seat.getId()).orElseThrow().getSeatStatus())
                .isEqualTo(SeatStatus.AVAILABLE);
        mockMvc.perform(
                        post("/api/customer/bookings/{id}/confirm", bookingId)
                                .header("Authorization", bearer(context.customerToken())))
                .andExpect(status().isConflict());
        mockMvc.perform(
                        post("/api/customer/bookings/{id}/cancel", bookingId)
                                .header("Authorization", bearer(context.customerToken())))
                .andExpect(status().isConflict());
    }
    ...
    void everyCustomerBookingEndpointRequiresAuthentication() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/customer/bookings")
                                ...
                                .content(json(Map.of("showId", 1, "seatIds", List.of(1)))))
                .andExpect(status().isUnauthorized());
        mockMvc.perform(post("/api/customer/bookings/1/confirm"))
                .andExpect(status().isUnauthorized());
        ...
        mockMvc.perform(post("/api/customer/bookings/1/cancel"))
                .andExpect(status().isUnauthorized());
    }
    ...
    void customerDetailIsOwnerOnlyAndDoesNotExposeManagementFields() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/customer/bookings/{bookingId}", bookingId)
                                .header("Authorization", bearer(context.customerToken())))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                        post("/api/customer/bookings/{bookingId}/confirm", bookingId)
                                .header("Authorization", bearer(otherToken)))
                .andExpect(status().isForbidden());
        mockMvc.perform(
                        post("/api/customer/bookings/{bookingId}/cancel", bookingId)
                                .header("Authorization", bearer(otherToken)))
                .andExpect(status().isForbidden());
    }
    ...
    void customerHistoryUsesSummaryContractAndDeterministicSeatOrdering() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.data.content[0].selectedSeatCodes[0]")
                                .value(seats.get(0).getSeatCode()))
    ...
    void staffAndAdminUseSeparateManagementEndpoints() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/staff/bookings/{bookingId}", bookingId)
                                .header("Authorization", bearer(staffToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.customerEmail").value("management-detail@example.com"))
                .andExpect(jsonPath("$.data.createdAt").exists());
        mockMvc.perform(
                        get("/api/admin/bookings/{bookingId}", bookingId)
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.customerEmail").value("management-detail@example.com"))
    ...
    @Test
    void customerHistorySupportsPaginationStatusAndDateFiltersWithoutLeakingOtherUsers()
            throws Exception {
        ...
        createBooking(
                other.customerToken(),
                other.show().getId(),
                seatsForShow(other.show().getId()).getFirst().getId());
    ...
    void managementListsSupportSearchFiltersSortingAndSafeResponses() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/staff/bookings")
                                ...
                                .param("sortBy", "bookingTime")
                                .param("sortDir", "desc"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content[0].bookingId").value(bookingId))
                .andExpect(
                        jsonPath("$.data.content[0].customerEmail")
                                .value("aarav.management@example.com"))
        ...
        mockMvc.perform(
                        get("/api/admin/bookings")
                                ...
                                .param(
                                        "customerId",
                                        userRepository
                                                .findByEmail("aarav.management@example.com")
                                                .orElseThrow()
                                                .getId()
                                                .toString())
    ...
    void bookingListsRejectInvalidQueriesAndUnauthorizedRoles() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/staff/bookings")
                                .header("Authorization", bearer(staffToken))
                                .param("showDateFrom", "2026-09-02")
                                .param("showDateTo", "2026-09-01"))
                .andExpect(status().isBadRequest());
    ...
    void activeHoldBlocksUpdateButCancellationReleasesLockAndKeepsSeats() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/admin/shows/{id}", context.show().getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
                                                showRequest(
                                                        context.show().getMovie().getId(),
                                                        context.show().getHall().getId(),
                                                        11,
                                                        "10:00",
                                                        "12:00"))))
                .andExpect(status().isConflict());
        ...
        mockMvc.perform(
                        delete("/api/admin/shows/{id}", context.show().getId())
                                .header("Authorization", bearer(adminToken)))
    ...
    void showCancellationCancelsInitiatedBookingAndRevokesConfirmedTickets() throws Exception {
        ...
        Long bookingId =
                createBooking(
                        initiated.customerToken(), initiated.show().getId(), initiatedSeat.getId());

        mockMvc.perform(
                        put("/api/admin/shows/{id}", initiated.show().getId())
                                .header("Authorization", bearer(initiatedAdmin))
                                .contentType("application/json")
                                .content(
                                        json(
                                                showRequest(
                                                        initiated.show().getMovie().getId(),
                                                        initiated.show().getHall().getId(),
                                                        11,
                                                        "10:00",
                                                        "12:00"))))
                .andExpect(status().isConflict());
        ...
        mockMvc.perform(
                        delete("/api/admin/shows/{id}", initiated.show().getId())
                                .header("Authorization", bearer(initiatedAdmin)))
                .andExpect(status().isNoContent());
        assertThat(bookingRepository.findById(bookingId).orElseThrow().getStatus())
                .isEqualTo(BookingStatus.CANCELLED);
        assertThat(bookingRepository.findById(bookingId).orElseThrow().getCancelledAt())
                .isNotNull();
        ...
        Long confirmedId =
                createBooking(
                        confirmed.customerToken(),
                        confirmed.show().getId(),
                        seatsForShow(confirmed.show().getId()).getFirst().getId());
        saveSuccessfulPayment(bookingRepository.findById(confirmedId).orElseThrow());
        mockMvc.perform(
                        post("/api/customer/bookings/{bookingId}/confirm", confirmedId)
                                .header("Authorization", bearer(confirmed.customerToken())))
                .andExpect(status().isOk());
        mockMvc.perform(
                        delete("/api/admin/shows/{id}", confirmed.show().getId())
                                .header("Authorization", bearer(confirmedAdmin)))
                .andExpect(status().isNoContent());
        assertThat(ticketRepository.findByBookingIdOrderByIssuedAtAsc(confirmedId))
                .allSatisfy(
                        ticket ->
                                assertThat(ticket.getStatus())
                                        .isEqualTo(
                                                com.chalchitraghar.modules.tickets.enums
    ...
    void holdsBookingsAndConfirmationCloseWhenShowHasStarted() throws Exception {
        ...
        Long bookingId =
                createBooking(context.customerToken(), context.show().getId(), first.getId());
    ...
    private void holdSeat(String token, Long showId, Long seatId) throws Exception {
        mockMvc.perform(
                        post("/api/customer/bookings/hold")
                                ...
                                .contentType("application/json")
                                .content(
                                        json(Map.of("showId", showId, "seatIds", List.of(seatId)))))
                .andExpect(status().isOk());
    }
