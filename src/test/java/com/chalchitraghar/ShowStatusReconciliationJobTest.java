...
class ShowStatusReconciliationJobTest {
    ...
    void jobPersistsStaleActiveStatuses() {
        ...
        Clock clock = Clock.fixed(Instant.parse("2026-07-11T06:30:00Z"), ZoneId.of("Asia/Kathmandu"));
        Show stale = Show.builder().showDate(LocalDate.of(2026, 7, 11))
                .showTime(LocalTime.of(10, 0)).endTime(LocalTime.of(11, 0)).build();
        stale.setStatus(ShowStatus.SCHEDULED);
        when(repository.findByStatusIn(List.of(ShowStatus.SCHEDULED, ShowStatus.RUNNING)))
                .thenReturn(List.of(stale));

        new ShowStatusReconciliationJob(repository, new ShowLifecycleService(clock, repository)).reconcileStatuses();
