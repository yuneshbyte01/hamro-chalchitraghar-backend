...
class CatalogApiIntegrationTest extends AbstractIntegrationTest {
    ...
    void publicMovieListSupportsPageAndSize() throws Exception {
        saveMovie(
                "Movie One", MovieStatus.NOW_SHOWING, "Drama", "Nepali", LocalDate.of(2026, 1, 1));
        saveMovie(
                "Movie Two", MovieStatus.NOW_SHOWING, "Drama", "Nepali", LocalDate.of(2026, 1, 2));
        saveMovie(
                "Movie Three",
    ...
    void publicMovieListRejectsInvalidQueryParameters() throws Exception {
        mockMvc.perform(get("/api/public/movies").param("sortBy", "password"))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Invalid sortBy. Allowed values: id, title, genre, language, releaseDate, status, createdAt, updatedAt, durationMinutes"));
        ...
        mockMvc.perform(get("/api/public/movies").param("status", "PLAYING"))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Invalid status. Allowed values: UPCOMING, NOW_SHOWING, ENDED"));
        ...
        mockMvc.perform(get("/api/public/movies").param("status", "ENDED"))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Public movie listing does not support status ENDED"));
        ...
        mockMvc.perform(get("/api/public/movies").param("releaseDateFrom", "2026/01/01"))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Parameter 'releaseDateFrom' has invalid type"));
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("releaseDateFrom must be on or before releaseDateTo"));
    }
    ...
    void publicMovieDetailForEndedMovieReturnsNotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/public/movies/{id}", movie.getId()))
                ...
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(
                        jsonPath("$.message").value("Movie not found with id: " + movie.getId()));
    }
    ...
    void adminCanListAndFetchEndedMovies() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/admin/movies/{id}", ended.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
    ...
    void adminMovieDetailExposesAuditFields() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/admin/movies/{id}", movie.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("Movie fetched successfully"))
    ...
    void adminCanCreateMovie() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/admin/movies")
                                ...
                                .content(
                                        json(movieRequest("Admin Movie", MovieStatus.NOW_SHOWING))))
    ...
    void movieStatusTransitionsFollowLifecycle() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/admin/movies/{id}", upcomingToNowShowing.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        Movie nowShowingToEnded =
                saveMovie(
                        ...
                        LocalDate.now().minusDays(1));
        mockMvc.perform(
                        put("/api/admin/movies/{id}", nowShowingToEnded.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        Movie upcomingToEnded =
                saveMovie(
                        ...
                        LocalDate.now().plusDays(5));
        mockMvc.perform(
                        put("/api/admin/movies/{id}", upcomingToEnded.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        Movie endedToNowShowing =
                saveMovie(
                        ...
                        LocalDate.now().minusDays(1));
        mockMvc.perform(
                        put("/api/admin/movies/{id}", endedToNowShowing.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Invalid movie status transition from ENDED to NOW_SHOWING"));

        Movie endedToUpcoming =
                saveMovie(
                        "Ended To Upcoming",
                        MovieStatus.ENDED,
                        "Drama",
                        ...
                        LocalDate.now().minusDays(1));
        mockMvc.perform(
                        put("/api/admin/movies/{id}", endedToUpcoming.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Invalid movie status transition from ENDED to UPCOMING"));

        Movie nowShowingToUpcoming =
                saveMovie(
                        ...
                        LocalDate.now().minusDays(1));
        mockMvc.perform(
                        put("/api/admin/movies/{id}", nowShowingToUpcoming.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Invalid movie status transition from NOW_SHOWING to UPCOMING"));
    }
    ...
    void deleteMovieWithoutFutureActiveShowsSetsStatusEnded() throws Exception {
        ...
        mockMvc.perform(
                        delete("/api/admin/movies/{id}", movie.getId())
                                .header("Authorization", bearer(adminToken)))
    ...
    void deleteMovieWithFutureScheduledShowReturnsConflict() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Cannot end movie while future active shows exist"));
    }
    ...
    void deleteMovieAllowsPastCompletedAndCancelledShows() throws Exception {
        ...
        mockMvc.perform(
                        delete("/api/admin/movies/{id}", completedMovie.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isNoContent());

        Movie cancelledMovie =
                saveMovie(
                        "Delete Cancelled Movie",
                        MovieStatus.NOW_SHOWING,
                        "Drama",
                        "Nepali",
                        LocalDate.now());
        ...
        mockMvc.perform(
                        delete("/api/admin/movies/{id}", cancelledMovie.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isNoContent());
    }
    ...
    void updateMovieToEndedRespectsFutureActiveShows() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/admin/movies/{id}", blockedMovie.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        Movie allowedMovie =
                saveMovie(
                        ...
                        LocalDate.now());
        mockMvc.perform(
                        put("/api/admin/movies/{id}", allowedMovie.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        mockMvc.perform(
                        post("/api/admin/movies")
                                ...
                                .content(
                                        json(
                                                movieRequest(
                                                        "Blocked Movie", MovieStatus.NOW_SHOWING))))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.success").value(false))
    ...
    void customerStaffAndUnauthenticatedUsersCannotAccessAdminMovieEndpoints() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/movies").header("Authorization", bearer(customerToken)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Access denied"));

        mockMvc.perform(get("/api/admin/movies").header("Authorization", bearer(staffToken)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Access denied"));
    ...
    void publicHallEndpointsExposeOnlyActivePublicContracts() throws Exception {
        ...
        mockMvc.perform(get("/api/public/halls/{id}", inactiveHall.getId()))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Hall not found with id: " + inactiveHall.getId()));
    }
    ...
    void publicHallListSupportsPagingSearchAndSorting() throws Exception {
        ...
        mockMvc.perform(get("/api/public/halls").param("search", "gamma"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content.length()").value(1))
                .andExpect(jsonPath("$.data.content[0].name").value("Gamma Hall"));

        mockMvc.perform(get("/api/public/halls").param("search", "inactive"))
                .andExpect(status().isOk())
    ...
    void publicHallListRejectsInvalidSorting() throws Exception {
        mockMvc.perform(get("/api/public/halls").param("sortBy", "password"))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Invalid sortBy. Allowed values: id, name, capacity, layoutRef, status, createdAt, updatedAt"));
    ...
    void adminHallEndpointsExposeAdminContractsAndAllStatuses() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/halls").header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/admin/halls/active").header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.length()").value(1))
                .andExpect(jsonPath("$.data[0].id").value(activeHall.getId()))
        ...
        mockMvc.perform(
                        get("/api/admin/halls/{id}", inactiveHall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath(
                                "$.data.content[*].id",
                                hasItems(
                                        alpha.getId().intValue(),
                                        beta.getId().intValue(),
                                        inactive.getId().intValue())));
    }
    ...
    void adminHallListRejectsInvalidFiltersAndSorting() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Invalid status. Allowed values: ACTIVE, INACTIVE"));
    ...
    void nonAdminsCannotUseAdminHallEndpoints() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/halls").header("Authorization", bearer(customerToken)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access denied"));

        mockMvc.perform(get("/api/admin/halls").header("Authorization", bearer(staffToken)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.message").value("Access denied"));
    ...
    void duplicateHallNameValidationIsCaseInsensitiveAndAppliesOnUpdate() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/admin/halls/{id}", other.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(json(hallRequest("UNIQUE HALL", Status.ACTIVE))))
                .andExpect(status().isConflict())
        ...
        mockMvc.perform(
                        put("/api/admin/halls/{id}", existing.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(json(hallRequest(" unique hall ", Status.ACTIVE))))
                .andExpect(status().isOk())
    ...
    void hallStatusLifecycleAllowsCreateAndToggleBetweenActiveAndInactive() throws Exception {
        ...
        Long inactiveHallId =
                objectMapper
                        .readTree(inactiveResult.getResponse().getContentAsString())
                        .path("data")
                        .path("id")
                        .asLong();

        mockMvc.perform(
                        put("/api/admin/halls/{id}", inactiveHallId)
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
    ...
    void hallInactivationChecksFutureActiveShowsAndUpdatesPublicVisibility() throws Exception {
        ...
        mockMvc.perform(
                        delete("/api/admin/halls/{id}", blockedHall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isConflict())
                .andExpect(
                        jsonPath("$.message")
                                .value("Cannot inactivate hall while future active shows exist"));

        mockMvc.perform(
                        put("/api/admin/halls/{id}", blockedHall.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        mockMvc.perform(
                        put("/api/admin/halls/{id}", freeHall.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
    ...
    void hallDeleteInactivatesWhenNoFutureShowsExist() throws Exception {
        ...
        mockMvc.perform(
                        delete("/api/admin/halls/{id}", hall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isNoContent());

        mockMvc.perform(
                        get("/api/admin/halls/{id}", hall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("INACTIVE"));
    }
    ...
    void hallLayoutFieldsCannotChangeAfterSeatTemplatesExistButNameCanChange() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/admin/halls/{id}", hall.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
                                                hallRequest(
                                                        "Template Locked Hall",
                                                        200,
                                                        "standard",
                                                        Status.ACTIVE))))
                .andExpect(status().isConflict())
    ...
    void seatLayoutGenerationRequiresActiveHallCapacity188AndNoExistingLayout() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/admin/halls/{hallId}/seat-layout", inactiveHall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isConflict())
                .andExpect(
                        jsonPath("$.message")
                                .value("Cannot generate seat layout for an inactive hall"));

        mockMvc.perform(
                        post("/api/admin/halls/{hallId}/seat-layout", wrongCapacityHall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isConflict())
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Seat layout generation currently requires hall capacity to be 188"));

        mockMvc.perform(
                        post("/api/admin/halls/{hallId}/seat-layout", validHall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
    ...
    void adminCanGenerateSeatLayout() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/admin/halls/{hallId}/seat-layout", hall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("Seat layout generated successfully"))
    ...
    void adminCanRetrieveGeneratedSeatLayout() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/admin/halls/{hallId}/seat-layout", hall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
    ...
    void adminCanSortTemplatesAndInvalidBrowseParametersReturnBadRequest() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Invalid seatType. Allowed values: PREMIUM, PLATINUM"));
        mockMvc.perform(
                        get("/api/admin/halls/{hallId}/seat-layout", hall.getId())
                                .header("Authorization", bearer(adminToken))
                                .param("sortBy", "price"))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Invalid sortBy. Allowed values: positionIndex, rowLabel, seatNumber, seatCode, seatType"));
        mockMvc.perform(
                        get("/api/admin/halls/{hallId}/seat-layout", hall.getId())
                                .header("Authorization", bearer(adminToken))
                                .param("sortDir", "sideways"))
    ...
    void seatLayoutRetrievalReturnsNotFoundForHallWithoutLayoutAndMissingHall() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Seat layout not found with id: " + hall.getId()));

        mockMvc.perform(
                        get("/api/admin/halls/{hallId}/seat-layout", Long.MAX_VALUE)
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isNotFound())
                .andExpect(
                        jsonPath("$.message").value("Hall not found with id: " + Long.MAX_VALUE));
    }
    ...
    void generatedTemplatePresetHasExpectedBoundsAndSequentialPositions() throws Exception {
        ...
        org.assertj.core.api.Assertions.assertThat(templates.getFirst().getSeatCode())
                .isEqualTo("A1");
        org.assertj.core.api.Assertions.assertThat(templates.getLast().getSeatCode())
                .isEqualTo("J20");
        for (int index = 0; index < templates.size(); index++) {
            org.assertj.core.api.Assertions.assertThat(templates.get(index).getPositionIndex())
                    .isEqualTo(index);
        }
    }
    ...
    void adminCanRegenerateLayoutOnlyWithoutShowDependencies() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/admin/halls/{hallId}/seat-layout/regenerate", hall.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Cannot regenerate seat layout while shows exist for this hall"));
    }
    ...
    void regenerationRejectsInactiveMissingLayoutAndNonAdmins() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/admin/halls/{hallId}/seat-layout/regenerate", inactive.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isConflict());
        mockMvc.perform(
                        post(
                                        "/api/admin/halls/{hallId}/seat-layout/regenerate",
                                        withoutLayout.getId())
                                .header("Authorization", bearer(adminToken)))
                .andExpect(status().isNotFound());
        mockMvc.perform(
    ...
    void showHallChangeIsAllowedBeforeSeatsAndBlockedAfterSeats() throws Exception {
        ...
        Show showWithoutSeats =
                showRepository.save(
                        Show.builder()
                                .movie(movie)
                                .hall(firstHall)
                                .showDate(LocalDate.now().plusDays(20))
                                .showTime(LocalTime.of(10, 0))
                                .endTime(LocalTime.of(12, 0))
                                .build());

        mockMvc.perform(
                        put("/api/admin/shows/{id}", showWithoutSeats.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
        ...
        Show showWithSeats =
                saveShowWithSeats(
                        saveMovie("Hall Change Generated Movie", MovieStatus.NOW_SHOWING),
                        firstHall,
                        adminToken);
        mockMvc.perform(
                        put("/api/admin/shows/{id}", showWithSeats.getId())
                                .header("Authorization", bearer(adminToken))
                                .contentType("application/json")
                                .content(
                                        json(
                                                showRequest(
                                                        showWithSeats.getMovie().getId(),
                                                        secondHall.getId(),
                                                        10,
                                                        "10:00",
                                                        "12:00"))))
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Cannot change show hall after seats have been generated"));
    ...
    @Test
    void publicSeatRetrievalRejectsMissingCancelledAndCompletedShowsAndNormalizesExpiredLocks()
            throws Exception {
    ...
    void showCreationRejectsUpcomingEndedInactiveAndOverlappingShows() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Hall is already booked for another show during this time period, including the 15-minute cleaning buffer."))
                .andExpect(jsonPath("$.errors").isArray());
    }
    ...
    private void assertInvalidMovieRequest(String adminToken, Map<String, Object> request)
            throws Exception {
        mockMvc.perform(
                        post("/api/admin/movies")
                                .header("Authorization", bearer(adminToken))
