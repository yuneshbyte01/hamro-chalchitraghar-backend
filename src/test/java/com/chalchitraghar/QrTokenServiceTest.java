...
class QrTokenServiceTest {
    ...
    @Test
    void tokensHaveAtLeast256BitsAndAreUniqueAndHashMatches() {
        var encryption = new QrTokenEncryptionServiceImpl(properties());
        var service = new QrTokenServiceImpl(encryption, properties());
    ...
    @Test
    void authenticatedEncryptionRoundTripsUsesRandomIvAndRejectsTamperingAndWrongKey() {
        var service = new QrTokenEncryptionServiceImpl(properties());
        String raw = "opaque-token";
        ...
        var wrong =
                new TicketQrProperties(
                        ...
                        60,
                        30);
        assertThatThrownBy(() -> new QrTokenEncryptionServiceImpl(wrong).decrypt(first))
                .isInstanceOf(IllegalStateException.class);
    }

    @Test
    void invalidKeyConfigurationFailsFast() {
