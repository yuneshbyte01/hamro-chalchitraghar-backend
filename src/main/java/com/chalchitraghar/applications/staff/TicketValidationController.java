...
public class TicketValidationController {
 ...
 @GetMapping("/tickets") @Operation(summary="Search tickets for admission operations") public ResponseEntity<ApiResponse<com.chalchitraghar.shared.response.PageResponse<StaffTicketSummaryResponse>>> list(@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="20")int size,@RequestParam(defaultValue="issuedAt")String sortBy,@RequestParam(defaultValue="desc")String sortDir,@RequestParam(required=false)String search,@RequestParam(required=false)String status,@RequestParam(required=false)Long showId,@RequestParam(required=false)Long movieId,@RequestParam(required=false)Long hallId,@RequestParam(required=false)String bookingReference){var c=new com.chalchitraghar.modules.tickets.dto.request.TicketSearchCriteria(search,status,showId,movieId,hallId,bookingReference,null,null,null,null,null,null,null,null,null);return ResponseEntity.ok(ApiResponse.success("Tickets fetched successfully",queries.staff(c,page,size,sortBy,sortDir)));}
}
