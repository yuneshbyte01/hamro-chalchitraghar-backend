...
public class AdminSeatLayoutController {
    ...
    @Operation(summary = "Get generated seat layout for a hall",
            description = "Returns generated seat templates with optional case-insensitive search, filters, and sorting. Statistics describe the returned templates.")
    ...
    public ResponseEntity<ApiResponse<AdminSeatLayoutResponse>> getSeatLayout(
            ...
            @Parameter(description = "Sort field: positionIndex, rowLabel, seatNumber, seatCode, seatType")
            @RequestParam(defaultValue = "positionIndex") String sortBy,
            @Parameter(description = "Sort direction: asc or desc")
            @RequestParam(defaultValue = "asc") String sortDir) {
        return ResponseEntity.ok(ApiResponse.success(
                "Seat layout fetched successfully",
                seatLayoutService.getSeatLayout(
                        hallId, new SeatTemplateSearchCriteria(search, seatType, row, sortBy, sortDir))));
    }
