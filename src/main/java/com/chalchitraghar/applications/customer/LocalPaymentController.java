...
@Tag(name = "Local Payments", description = "Development/test-only LOCAL payment simulator")
...
public class LocalPaymentController {
    ...
    @Operation(
            summary = "Simulate LOCAL payment result",
            description =
                    "Enabled only when PAYMENT_LOCAL_ENABLED=true; never enabled by default in production.")
    public ResponseEntity<ApiResponse<CustomerPaymentDetailResponse>> process(
            @PathVariable String reference,
            @Valid @RequestBody LocalPaymentProcessRequest request) {
        User u = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Local payment processed successfully",
                        service.processLocal(reference, request, u)));
    }
}
