...
public class ProfileController {
    ...
    public ResponseEntity<ApiResponse<UserResponse>> getProfile() {
        ...
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Profile fetched successfully", userMapper.toResponseDto(currentUser)));
    }
    ...
    @Operation(
            ...
            description =
                    "Updates only the authenticated user's display name. Email and role cannot be changed here.")
    public ResponseEntity<ApiResponse<UserResponse>> updateProfile(
            @Valid @RequestBody ProfileUpdateRequest request) {
        User updatedUser =
                userService.updateCurrentUserProfile(getCurrentUser(), request.getName());
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Profile updated successfully", userMapper.toResponseDto(updatedUser)));
    }
    ...
    @Operation(
            ...
            description =
                    "Verifies the current password, validates the new strong password, and stores it as a BCrypt hash.")
    public ResponseEntity<ApiResponse<Void>> changePassword(
            @Valid @RequestBody ChangePasswordRequest request) {
        userService.changeCurrentUserPassword(
                getCurrentUser(), request.getCurrentPassword(), request.getNewPassword());
