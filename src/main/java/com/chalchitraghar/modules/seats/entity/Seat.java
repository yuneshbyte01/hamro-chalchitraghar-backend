...
@Table(
        ...
        uniqueConstraints = {
            @jakarta.persistence.UniqueConstraint(
                    name = "uk_seats_show_code",
                    columnNames = {"show_id", "seat_code"}),
            @jakarta.persistence.UniqueConstraint(
                    name = "uk_seats_show_row_number",
                    columnNames = {"show_id", "row_label", "seat_number"}),
            @jakarta.persistence.UniqueConstraint(
                    name = "uk_seats_show_position",
                    columnNames = {"show_id", "position_index"})
        })
@Data
...
public class Seat extends GenericEntity {
    ...
    /** Lifecycle callback invoked before entity persistence. Sets default status to AVAILABLE. */
    @PrePersist
    @Override
