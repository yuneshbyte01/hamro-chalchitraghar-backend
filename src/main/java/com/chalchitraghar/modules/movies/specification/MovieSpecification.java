...
public final class MovieSpecification {
    ...
    public static Specification<Movie> search(
            MovieSearchCriteria criteria, MovieStatus status, boolean includeEnded) {
            ...
            if (criteria.genre() != null && !criteria.genre().isBlank()) {
                predicates.add(
                        cb.equal(
                                ...
                                criteria.genre().trim().toLowerCase()));
            }
            ...
            if (criteria.releaseDateFrom() != null) {
                predicates.add(
                        cb.greaterThanOrEqualTo(
                                root.get("releaseDate"), criteria.releaseDateFrom()));
            }
            if (criteria.releaseDateTo() != null) {
                predicates.add(
                        cb.lessThanOrEqualTo(root.get("releaseDate"), criteria.releaseDateTo()));
            }
