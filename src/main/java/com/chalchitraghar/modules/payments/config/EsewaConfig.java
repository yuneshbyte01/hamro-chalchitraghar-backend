...
public class EsewaConfig {
    @Bean
    RestClient esewaRestClient(EsewaProperties p) {
        var f = new SimpleClientHttpRequestFactory();
        f.setConnectTimeout(Duration.ofMillis(p.connectTimeoutMs()));
        f.setReadTimeout(Duration.ofMillis(p.readTimeoutMs()));
