...
public class PaymentServiceImpl implements PaymentService {
    ...
    public List<CustomerPaymentSummaryResponse> getMyPayments(User user) {
        return paymentRepository.findByBookingUserIdOrderByCreatedAtDesc(user.getId()).stream()
    ...
    public CustomerPaymentDetailResponse getMyPaymentByReference(String reference, User user) {
        ...
        if (!payment.getBooking().getUser().getId().equals(user.getId()))
            throw paymentNotFound(reference);
        return mapper.toCustomerDetail(payment);
    }

    public List<CustomerPaymentSummaryResponse> getMyPaymentsByBookingReference(
    ...
    public StaffPaymentDetailResponse getStaffPaymentByReference(String reference) {
        return mapper.toStaffDetail(findPayment(reference));
    }

    public List<StaffPaymentSummaryResponse> getStaffPaymentsByBookingReference(String reference) {
        Booking booking = findBooking(reference);
        return paymentRepository.findByBookingIdOrderByCreatedAtDesc(booking.getId()).stream()
                .map(mapper::toStaffSummary)
                .toList();
    }

    public AdminPaymentDetailResponse getAdminPaymentByReference(String reference) {
        return mapper.toAdminDetail(findPayment(reference));
    }
    ...
    public Object initiate(
        ...
        Booking booking =
                bookingRepository
                        .findByIdForUpdate(findBooking(reference).getId())
                        .orElseThrow(() -> bookingNotFound(reference));
        if (!booking.getUser().getId().equals(user.getId())) throw bookingNotFound(reference);
        Payment prior =
                paymentRepository
                        .findByBookingIdAndIdempotencyKey(booking.getId(), key)
                        .orElse(null);
        if (prior != null) {
            if (prior.getProvider() == request.provider() && prior.getMethod() == request.method())
                return initiationResponse(prior);
            throw new PaymentConflictException(
                    "Idempotency key was already used with different payment parameters");
        }
        if (booking.getStatus() != BookingStatus.INITIATED)
            throw new PaymentConflictException("Only INITIATED bookings can start payment");
        if (booking.getExpiresAt() != null
                && !LocalDateTime.now(clock).isBefore(booking.getExpiresAt()))
            throw new PaymentConflictException("Booking has expired and cannot start payment");
        showLifecycleService.assertBookable(booking.getShow());
        var active =
                paymentRepository.findByBookingIdAndStatusIn(
                        booking.getId(), PaymentLifecycleService.ACTIVE);
        if (!active.isEmpty())
            throw new PaymentConflictException("Booking already has an active payment attempt");
        if (paymentRepository.existsByBookingIdAndStatus(booking.getId(), PaymentStatus.SUCCESS))
            throw new PaymentConflictException("Booking already has a successful payment");
        PaymentProviderAdapter adapter =
                providerAdapters.stream()
                        .filter(a -> a.supports(request.provider()))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        new PaymentConflictException(
                                                "Payment provider is not available"));
        if (request.method() != PaymentMethod.ONLINE)
            throw new PaymentConflictException("Payment method is not available");
        if (request.provider() == PaymentProvider.ESEWA && !esewaProperties.enabled())
            throw new PaymentConflictException("eSewa payments are disabled");
    ...
    private Object initiationResponse(Payment p) {
        return p.getProvider() == PaymentProvider.ESEWA
                ? esewaPayloadFactory.create(p)
                : mapper.toCustomerDetail(p);
    }
    ...
    public CustomerPaymentDetailResponse cancel(String reference, User user) {
        ...
        lifecycle.transition(p, PaymentStatus.CANCELLED);
    ...
    public CustomerPaymentDetailResponse processLocal(
        ...
        Payment p = findPaymentForUpdate(reference, user);
        lifecycle.reconcileExpiry(p);
        if (p.getProvider() != PaymentProvider.LOCAL)
            throw new PaymentConflictException("Only LOCAL payments can be processed locally");
        PaymentStatus target =
                request.result() == LocalPaymentProcessRequest.LocalPaymentResult.SUCCESS
    ...
    private Payment findPaymentForUpdate(String ref, User user) {
        Payment p =
                paymentRepository
                        .findByPaymentReferenceForUpdate(normalize(ref))
                        .orElseThrow(() -> paymentNotFound(ref));
        if (!p.getBooking().getUser().getId().equals(user.getId())) throw paymentNotFound(ref);
        return p;
    }
    ...
    private Payment findPayment(String ref) {
        return paymentRepository
                .findByPaymentReference(normalize(ref))
                .orElseThrow(() -> paymentNotFound(ref));
    }

    private Booking findBooking(String ref) {
        return bookingRepository
                .findByBookingReference(normalize(ref))
                .orElseThrow(() -> bookingNotFound(ref));
    }
    ...
    private ResourceNotFoundException paymentNotFound(String ref) {
        return new ResourceNotFoundException("Payment not found with reference: " + ref);
    }

    private ResourceNotFoundException bookingNotFound(String ref) {
        return new ResourceNotFoundException("Booking not found with reference: " + ref);
    }
}
