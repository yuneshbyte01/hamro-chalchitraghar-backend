...
public class EsewaSignatureService {
    ...
    public EsewaSignatureService(EsewaProperties p) {
        this.properties = p;
    }
    ...
    public String sign(String message) {
        try {
            ...
            throw new IllegalStateException("Could not sign eSewa request", e);
        }
    }

    public String requestSignature(BigDecimal total, String uuid) {
        return sign(
                "total_amount="
                        + amount(total)
                        + ",transaction_uuid="
    ...
    public void verify(String message, String supplied) {
        byte[] a = sign(message).getBytes(StandardCharsets.US_ASCII);
        byte[] b = supplied.getBytes(StandardCharsets.US_ASCII);
        if (!MessageDigest.isEqual(a, b))
            throw new PaymentConflictException("Invalid eSewa response signature");
    }
}
