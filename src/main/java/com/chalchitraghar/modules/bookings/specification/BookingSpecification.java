...
public final class BookingSpecification {
    ...
    public static Specification<Booking> search(
        ...
        return (root, query, cb) -> {
            ...
            if (criteria.showId() != null) predicates.add(cb.equal(show.get("id"), criteria.showId()));
            if (criteria.movieId() != null) predicates.add(cb.equal(movie.get("id"), criteria.movieId()));
            if (criteria.hallId() != null) predicates.add(cb.equal(hall.get("id"), criteria.hallId()));
            if (criteria.customerId() != null) predicates.add(cb.equal(user.get("id"), criteria.customerId()));
            if (criteria.showDateFrom() != null) predicates.add(cb.greaterThanOrEqualTo(show.get("showDate"), criteria.showDateFrom()));
            if (criteria.showDateTo() != null) predicates.add(cb.lessThanOrEqualTo(show.get("showDate"), criteria.showDateTo()));
            if (criteria.bookingTimeFrom() != null) predicates.add(cb.greaterThanOrEqualTo(root.get("bookingTime"), criteria.bookingTimeFrom()));
            if (criteria.bookingTimeTo() != null) predicates.add(cb.lessThanOrEqualTo(root.get("bookingTime"), criteria.bookingTimeTo()));
