...
@Entity @Table(name="ticket_deliveries",uniqueConstraints=@UniqueConstraint(name="uk_ticket_deliveries_booking_channel",columnNames={"booking_id","channel"}))
...
public class TicketDelivery extends GenericEntity {
 ...
 @Enumerated(EnumType.STRING) @Column(nullable=false,length=30) private TicketDeliveryChannel channel;
 @Column(nullable=false) private String recipient;
 @Enumerated(EnumType.STRING) @Column(nullable=false,length=30) private TicketDeliveryStatus status;
 @Column(nullable=false) private int attemptCount;
 private LocalDateTime lastAttemptAt; private LocalDateTime sentAt; @Column(length=500) private String failureMessage;
}
