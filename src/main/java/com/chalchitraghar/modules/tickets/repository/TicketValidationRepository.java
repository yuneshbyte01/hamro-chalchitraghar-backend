...
public interface TicketValidationRepository
        extends JpaRepository<TicketValidation, Long>,
                org.springframework.data.jpa.repository.JpaSpecificationExecutor<TicketValidation> {
    @EntityGraph(attributePaths = {"validatedBy"})
    List<TicketValidation> findByTicketIdOrderByValidationTimeDesc(Long ticketId);

    List<TicketValidation> findByValidationTimeBetweenOrderByValidationTimeDesc(
