...
public class SeatLayoutServiceImpl implements SeatLayoutService {
    ...
    private static final Set<String> TEMPLATE_SORT_FIELDS =
            Set.of("positionIndex", "rowLabel", "seatNumber", "seatCode", "seatType");
    ...
    public AdminSeatLayoutResponse getSeatLayout(Long hallId, SeatTemplateSearchCriteria criteria) {
        ...
        List<SeatTemplate> templates =
                seatTemplateRepository.findByHallIdOrderByPositionIndexAsc(hallId);
        if (templates.isEmpty()) {
            throw new ResourceNotFoundException("Seat layout", hallId);
        }
        SeatTemplateSearchCriteria effectiveCriteria =
                criteria == null
        ...
        Comparator<SeatTemplate> comparator =
                templateComparator(effectiveCriteria.sortBy(), effectiveCriteria.sortDir());
        String search = normalize(effectiveCriteria.search());
        String row = normalize(effectiveCriteria.row());

        List<SeatTemplate> filteredTemplates =
                templates.stream()
                        .filter(
                                template ->
                                        search == null
                        ...
                        .filter(
                                template ->
                                        row == null || template.getRowLabel().equalsIgnoreCase(row))
                        .sorted(comparator)
                        .toList();
    ...
    private SeatType parseSeatType(String value) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Invalid seatType. Allowed values: PREMIUM, PLATINUM");
        }
    }
                ...
                switch (field) {
                    case "rowLabel" ->
                            Comparator.comparing(
                                    SeatTemplate::getRowLabel, String.CASE_INSENSITIVE_ORDER);
                    case "seatNumber" -> Comparator.comparing(SeatTemplate::getSeatNumber);
                    case "seatCode" ->
                            Comparator.comparing(
                                    SeatTemplate::getSeatCode, String.CASE_INSENSITIVE_ORDER);
                    case "seatType" ->
                            Comparator.comparing(template -> template.getSeatType().name());
    ...
    private List<SeatTemplate> buildValidatedTemplates(Hall hall) {
        ...
        for (char row = 'B'; row <= 'J'; row++) {
            for (int seat = 1; seat <= 20; seat++) {
                templates.add(
                        buildSeat(hall, String.valueOf(row), seat, SeatType.PLATINUM, index++));
            }
        }
    ...
    private SeatTemplate buildSeat(
            Hall hall, String rowLabel, int seatNumber, SeatType seatType, int positionIndex) {
