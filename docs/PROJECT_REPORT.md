...
Payment records are currently created only by repository/test setup. No payment initiation, provider verification, eSewa/Khalti calls, Stripe, callbacks, cash collection, refunds, reconciliation, or payment-driven booking confirmation is implemented. Booking confirmation continues to use the existing permissive local `PaymentAuthorizationService` compatibility adapter.
