...
Payment-1 exposes persisted payment history only. It does not initiate, verify, collect, refund, or reconcile payments, and it does not integrate any external provider. Payment references are generated by the server in `PAY-YYYYMMDD-XXXXXXXX` format.
